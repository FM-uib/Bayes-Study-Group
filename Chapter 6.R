...
mass <- c(6,8,11, 5,7,11, 6,9,11)
pop <- factor(c(1,1,1, 2,2,2, 3,3,3))
svl <- c(40, 45,53, 39,50,61, 41,52,57)

# Main effects model 
# effect parameterization
X <- model.matrix(~ pop + svl)
# mean parameterization
Xm <- model.matrix(~ pop + svl -1)

fm <- lm(mass ~ X-1)
plot(svl, mass, col = c(rep("red", 3), rep("blue", 3), rep("green", 3)))
abline(fm$coef[1], fm$coef[4], col = "red")
abline(fm$coef[1]+ fm$coef[2], fm$coef[4], col = "blue")
abline(fm$coef[1]+ fm$coef[3], fm$coef[4], col = "green")
